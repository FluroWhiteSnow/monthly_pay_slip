...
class CalculatePay extends User {
  ...
  calcAnnualTax() {
    ...
    return (this.annualTax = tax.toFixed(2));
  }

  calcNetAnnunal() {
    this.calcAnnualTax();
    let netAnnual = this.salary - this.annualTax;
    return (this.netAnnual = netAnnual.toFixed(2));
  }

  calcNetMontly() {
    let netMonthly = this.netAnnual / 12;
    return (this.netMonthly = netMonthly.toFixed(2));
  }

  calcGrossMonthly() {
    let grossMonthly = this.salary / 12;
    return (this.grossMonthly = grossMonthly.toFixed(2));
  }

  calcMonthlyTax() {
    let monthlyTax = this.annualTax / 12;
    return (this.monthlyTax = monthlyTax.toFixed(2));
  }
}
module.exports = {
  CalculatePay,
